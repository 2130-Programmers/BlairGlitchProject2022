...
public class SwerveMotor extends SubsystemBase {
  ...
  // what is done here with the @param is we are adding a comment to our method so
  // that when you call to it it will say
  // what each parameter will be, you can also say @return ("return type") to say
  // which return type it will be
  /**
  ...
  public SwerveMotor(int rotate, int drive, int encoder) {
    // when calling swerve motor we put in parameters which will assign these motors
    // and encoders numbers and ports
  ...
  // gets the value of an encoder from 0 to 4096, we transfer negetive numbers to
  ...
  /**
   * Takes a joystick input and turns the rotation motor to the equivilant of the
   * position in ticks
   * 
  ...
  public void pointToTarget(double target) {
    ...
    if ((encoderRemainingValue % ticksInRotation != 0) || (-encoderRemainingValue % ticksInRotation != 0)) {
      ...
      if (encoderRemainingValue > ticksInThreeQuarters) {
      ...
      } else if (encoderRemainingValue < ticksInHalf && encoderRemainingValue > -ticksInHalf
        ...
        directionMultiplier = Math
            .round((encoderRemainingValue - ticksInRotation) / Math.abs(encoderRemainingValue - ticksInRotation)) * -1;// +
      } else if (encoderRemainingValue < -ticksInHalf) {
        directionMultiplier = Math
            .round((encoderRemainingValue + ticksInRotation) / Math.abs(encoderRemainingValue + ticksInRotation)) * -1;// +
      } else {
      ...
      // goes towards the point, if it is outside the large error it goes fast, if it
      // is
      // in that range it goes at the slow speed untill smaller than the small error

      // turnPowerRatio = .000004 * Math.pow(encoderRemainingValue, 2) + .00002 *
      // encoderRemainingValue +.0202;
  ...
  public void drive(double speed, double angle, double offset) {
    ...
    // POINTSETMOD is not being used, it swaps the polarity of the desired target
    // and makes it count up from the bottom isntead of the top
